...
namespace dx9 {
	...
	bool DirectXFont::Create(const wchar_t * fontName, size_t fontSize, FontWeight fontWeight, bool isItalic, bool isUnderLine, bool isStrikeOut, AntialiasLevel level) {
		DeleteAll();
	...
	bool DirectXFont::Draw(float x, float y, DWORD color, const wchar_t* s, ...) {
	...
	bool DirectXFont::DrawInRect(RectF& rect, DWORD format, DWORD color, const wchar_t * s, ...) {
		va_list vlist;
		va_start(vlist, s);
	...
	bool DirectXFont::Draw(float x, float y, DWORD color, size_t startCharCnt, int drawCharCnt, const wchar_t * s, ...) {
		RectF rect = {x, y, 0.0f, 0.0f};
	...
	bool DirectXFont::DrawInRect(RectF & rect, DWORD format, DWORD color, size_t startCharCnt, int drawCharCnt, const wchar_t * s, ...) {
		va_list vlist;
		va_start(vlist, s);
	...
	bool DirectXFont::Draw(float x, float y, DWORD color, float rotateDeg, FontRotOrigin rotOrigin, const wchar_t * s, ...) {
		RectF rect = {x, y, 0.0f, 0.0f};
	...
	bool DirectXFont::DrawInRect(RectF & rect, DWORD format, DWORD color, float rotateDeg, FontRotOrigin rotOrigin, const wchar_t * s, ...) {
		va_list vlist;
		va_start(vlist, s);
	...
	bool DirectXFont::DrawInRect(RectF & rect, DWORD format, DWORD color, size_t startCharCnt, int drawCharCnt, size_t fontSize, float letterSpace, float rotateDeg, FontRotOrigin rotOrigin, const wchar_t * s, ...) {
		va_list vlist;
		va_start(vlist, s);
	...
	bool DirectXFont::DrawFont(RectF & rect, DWORD format, DWORD color, size_t startCharCnt, int drawCharCnt, size_t fontSize, float letterSpace, float rotateDeg, FontRotOrigin rotOrigin, const wchar_t * s, va_list vlist) {
		if (!isDrawable()) {
			return false;
		}
		if (!hFont) {
		...
		wstring wstr(CHARACTER_MAXCNT, L'\0');
		int strLen = _vsnwprintf_s_l(&wstr[0], CHARACTER_MAXCNT+1, CHARACTER_MAXCNT, s, locale, vlist);
		_free_locale(locale);
		...
		int charCnt = CalcTextPosition(wstr, rect, inScale, letterSpace, format, pos, strArea, outScale);
		if (charCnt < 0) {
			return false;
		}
		...
		size_t endCharCnt = 0;
		if (drawCharCnt < 0 || startCharCnt + drawCharCnt > strLen) {
			endCharCnt = strLen;
		}
		else {
	...
	int DirectXFont::CalcTextPosition(const std::wstring &str, RectF & rect, float inScale, int letterSpace, DWORD format, std::vector<PointF>& pt, RectF & strArea, float & outScale) {
		...
		while (lineHead < str.size()) {
			...
			for (charCnt=lineHead; charCnt<=lineEnd; charCnt++) {
				// 文字コード取得
				UINT code = (UINT)(str[charCnt]);

				if (!texRes[code]) {
					continue;
				}

				pt[charCnt].x = rect.left + texRes[code]->_chInfo().originX + offsetX;
				pt[charCnt].y = rect.top - texRes[code]->_chInfo().originY + offsetY;

				offsetX += ( texRes[code]->_chInfo().AreaW*inScale + letterSpace)*outScale;
			}
	...
	bool DirectXFont::StoreFontTex(const std::wstring& wstr) {
		using namespace std;
		...
		for (wchar_t wch : wstr) {
			...
			UINT code = static_cast<UINT>(wch);
			...
			if (!texRes[code]) {
				texRes[code] = make_unique<FontTextureW>();
				if (!texRes[code]->Create(d3ddev9, hdc, wch, antialiasLv, &tm)) {
					texRes[code].reset();
					return false;
				}
			}
